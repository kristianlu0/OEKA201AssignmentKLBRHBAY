...
title: "assignment_r"
author:
  - name: Bertelsen, Fredrik-André Hov
    affiliation:
      - id: HIOF
        name: Østfold University of Applied Sciences
  - name: Yousefi, Arya
    affiliation:
      - ref: HIOF
  - name: Røijem, Bendik Strømme
    affiliation:
      - ref: HIOF
  - name: Lund, Kristian
    affiliation:
      - ref: HIOF
abstract: "This paper investigates the determinants of mortality rates across U.S. states using cross-sectional data. We employ simple regression, multiple regression, and dummy variable analysis to examine the relationship between socioeconomic factors, health-related behaviors, and mortality. Results indicate that age structure and cigarette consumption are the strongest predictors of mortality, while income alone explains very little variation."
editor: source
keywords:
  - assignment
  - R
...
```{r}
# 2. Simple regression - must come BEFORE the scatterplot
...
```{r}
# Prediction interval - replace X with your INCC value
predict(enkelreg, 
        newdata = data.frame(INCC = 854.1), 
        interval = "prediction", 
        level = 0.95)
```

This means that the model explains pretty much none of the variation of the mortality. That means that we can’t use income as a regressor for simple regression to explain what leads to a higher mortality rate. {{< pagebreak >}}
...
b)  
```{r}
model <- lm(MORT ~ INCC + EDU2 + TOBC + AGED, data = df)
summary(model)
confint(model)
```
...
```{r}
df$high_income <- ifelse(df$INCC > mean(df$INCC), 1, 0)
model_d1 <- lm(MORT ~ high_income, data = df)
summary(model_d1)
```
...
```{r}
# Model 2 : income dummy variables + controls
model_d2 <- lm(MORT ~ high_income + AGED + POV + HEXC, data = df)
summary(model_d2)
```
...
```{r}
# Model 3: Aged Alone
model_aged <- lm(MORT ~ AGED, data = df)
summary(model_aged)
```
...
```{r}
df$high_age <- ifelse(df$AGED > median(df$AGED), 1, 0)
model_d3 <- lm(MORT ~ high_age, data = df)
summary(model_d3)
```
...
#| echo: false
source("wordcount.R")
result <- count_words_in_qmd("assignment_r.qmd")
cat(result$message)
```
