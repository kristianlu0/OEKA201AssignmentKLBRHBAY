...
format: pdf editor: visual Keywords: \[assignment,R , Gretl, econmetrics\] ---
...
```{r}
#| eval: false
data$high_income <- ifelse(data$INCC > mean(data$INCC), 1, 0)
model_d1 <- lm(MORT ~ high_income, data = data)
summary(model_d1)
```
...
model_d2 <- lm(MORT ~ high_income + AGED + POV + HEXC, data = data)
summary(model_d2)
...
# Model 3: Aged Alone
model_aged <- lm(MORT ~ AGED, data = data)
summary(model_aged)
...
data$high_age <- ifelse(data$AGED > median(data$AGED),1, 0)
model_d3 <- lm(MORT ~ high_age, data=data)
summary(model_d3)
